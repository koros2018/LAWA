...
async def get_balance(user_id: str, *, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    """查询用户余额"""
...
async def get_transactions(user_id: str, *, current_user: User = Depends(get_current_user), limit: int = Query(20, ge=1, le=100), db: AsyncSession = Depends(get_db)):
    """查询交易历史"""
...
async def daily_summary(user_id: str, *, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    """当日金币汇总"""
    return await coin_agent.run({
