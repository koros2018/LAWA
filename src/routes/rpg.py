...
async def join_guild(req: JoinGuildRequest, current_user: User = Depends(get_current_user)):
    result = await guild_agent.run({"action": "join", "user_id": str(current_user.id), "guild_id": req.guild_id})
    if "error" in result:
        raise HTTPException(400, result["error"])
    return result
...
async def contribute(req: ContributeRequest, current_user: User = Depends(get_current_user)):
    result = await guild_agent.run({
        "action": "contribute",
        "user_id": str(current_user.id),
        "amount": req.amount,
